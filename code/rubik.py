...
def home_servos():
    global l_pos
    global r_pos
    setDirection_left_turn(90 + left_dreh_tune,1)
    setDirection_right_turn(90 + right_dreh_tune,1)
    time.sleep(SLEEP)
    regrip()    
...
def single_action(action):
    ...
    if action =="A":
        ...
        if l_pos != 0:
            ...
            setDirection_left_turn(0 + left_dreh_tune, sleep)
            l_pos = 0
        ...
        if l_pos != 180:
            if l_pos == 90:
                ...
                sleep = SLEEP_LONG_FAKTOR
            setDirection_left_turn(180 + left_dreh_tune, sleep)
            l_pos = 180
        ...
        if r_pos !=0:
            if r_pos == 90:
                ...
                sleep = SLEEP_LONG_FAKTOR
            setDirection_right_turn(0 + right_dreh_tune, sleep)
            r_pos = 0
        ...
        if r_pos != 180:
            if r_pos == 90:
                ...
                sleep = SLEEP_LONG_FAKTOR
            setDirection_right_turn(180 + right_dreh_tune, sleep)
            r_pos = 180
...
def onButtonEvent_plus(plus, event):
    ...
    global left_dreh_tune
    global right_grip_tune
    global right_dreh_tune
    global LOAD
    global SLEEP
    global message
    global regrip_stat
...
#                left_grip_tune = GRIPPER_MAX
            setDirection_left_grip(left_grip_tune)
        elif state_machine == 20: # left dreh wird eingestellt
            left_dreh_tune = left_dreh_tune + 2
 #           if left_dreh_tune + 180 > TURN_MAX:
 #               left_dreh_tune = TURN_MAX-180
            setDirection_left_turn(90 + left_dreh_tune,0.5)
  ...
  #              right_grip_tune = GRIPPER_MAX
            setDirection_right_grip(right_grip_tune)
        elif state_machine == 40:  #right dreh wird eingestellt
            right_dreh_tune = right_dreh_tune + 2
   #         if right_dreh_tune + 180 > TURN_MAX:
   #             right_dreh_tune = TURN_MAX-180
            setDirection_right_turn(90 + right_dreh_tune,0.5) 
...
def onButtonEvent_enter(enter, event):
    ...
    if event == BUTTON_PRESSED:
        ...
        if dummy == 0:
            ...
            f.write(str(left_dreh_tune) + "\n")
            f.write(str(right_grip_tune) + "\n")
            f.write(str(right_dreh_tune) + "\n")
            f.write(str(LOAD) + "\n")
            f.write(str(SLEEP)+ "\n")
    ...
    while i<100:
        ...
        if dummy == 0:
            ...
            with canvas(device) as draw:
                ...
                draw.text((5, 40), str(left_dreh_tune), font = font, fill="white")
            time.sleep(0.1)
            ...
            with canvas(device) as draw:
                ...
                draw.text((5, 40), str(right_dreh_tune), font = font, fill="white")
            time.sleep(0.1)
...
def setup():
    ...
    if os.path.exists(HOME + "tune_values.txt"):
        ...
        dummy  = f.readline()
        left_dreh_tune = int(dummy)
        dummy  = f.readline()
        right_grip_tune  = int(dummy)
        dummy  = f.readline()
        right_dreh_tune = int(dummy)
        dummy  = f.readline()
        LOAD = int(dummy)
        dummy = f.readline()
        SLEEP = float(dummy)
